...
> **狀態**：✅ 已實作上線（2026-07-06，B1–B5 老闆於 2026-07-05 全數核准，含 fail-open ×1.0 方案；隔離驗收 PASS）。
...
| # | 項目 | 紅線類別 | 狀態 |
|---|------|----------|------|
| B1 | 新增 `src/signals/taiex_confirm.py`；修改 `src/tasks.py` 的 `generate_signal` 接上係數 | 動 src/signals/ | ✅ 已核准並實作 |
| B2 | 係數表 v0（見 §3）與 fail-open 行為（資料抓不到 → ×1.0 + Discord 警告）| 策略參數 | ✅ 已核准（fail-open ×1.0）|
| B3 | 新外部資料源：TWSE OpenAPI（純 HTTP JSON，用現有 httpx，**不加任何套件**）| 外部依賴 | ✅ 已核准（實測見 §2）|
| B4 | S4 訊號入庫方式：復用現有 `save_trend_signal(symbol="TAIEX")`，**無 schema 變更** | 確認即可 | ✅ 已核准並實作 |
| B5 | CLAUDE.md 路由表加一行指向本檔 | 規則檔 | ✅ 已完成（scalper session 一併加入）|
...
2. **法人動向**：三大法人近 5 個交易日**合計買賣超金額**正負。
   ...
   **實測記錄（2026-07-06）**：
   - 可用端點：`https://www.twse.com.tw/rwd/zh/fund/BFI82U?dayDate=YYYYMMDD&type=day&response=json`
   - 交易日回傳：`{"stat":"OK","fields":["單位名稱","買進金額","賣出金額","買賣差額"],"data":[...,["合計","447,310,392,567","520,850,423,419","-73,540,030,852"]]}`——合計列在最後、金額為千分位字串、**單位元**（÷1e8 = 億元）
   - 非交易日回傳：`{"stat":"很抱歉，沒有符合條件的資料!"}` → 跳過該日往前走（非「失敗」）
   - `openapi.twse.com.tw/v1/fund/BFI82U` 回 302 redirect **不可用**，且僅提供最新單日，棄用
